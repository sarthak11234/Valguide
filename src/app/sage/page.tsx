...
export default function SageChatbot() {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    sendMessage({ 
      ...
      role: 'user',
      // @ts-ignore - appending custom field to message payload
      experienceLevel 
    });
    setInput("");
  };
  ...
  return (
    <div className="min-h-screen bg-[#0F1923] text-[#ECE8E1] p-8 font-sans">
      <div className="max-w-3xl mx-auto border-4 border-black p-6 relative bg-[radial-gradient(circle,#39FF14_1px,transparent_1px)] bg-[size:10px_10px] min-h-[750px] flex flex-col">
        {/* Comic Panel Header */}
        ...
        {!experienceLevel ? (
          /* ONBOARDING FLOW */
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-[#0F1923] border-4 border-black mt-8 shadow-inner relative overflow-hidden">
            <div className="absolute inset-0 bg-[linear-gradient(rgba(0,0,0,0.5),rgba(0,0,0,0.5))] z-0" pointer-events-none></div>
            <div className="relative z-10 w-full max-w-lg">
              <h2 className="text-3xl font-black uppercase tracking-widest text-[#00E5FF] mb-2 transform -skew-x-5 shadow-black text-shadow-sm">
                *SYSTEM INITIATED*
              </h2>
              <p className="text-xl mb-10 font-bold tracking-wider">WHAT IS YOUR FPS EXPERIENCE, RECRUIT?</p>
              
              <div className="flex flex-col gap-6">
                <button 
                  onClick={() => setExperienceLevel("Beginner")}
                  className="bg-white text-black border-4 border-black p-4 text-xl font-bold uppercase tracking-wider transform -skew-x-5 hover:bg-[#FF4655] hover:text-white transition-colors shadow-[6px_6px_0px_#00E5FF]"
                >
                ...
                <button 
                  ...
                  className="bg-white text-black border-4 border-black p-4 text-xl font-bold uppercase tracking-wider transform -skew-x-5 hover:bg-[#39FF14] transition-colors shadow-[6px_6px_0px_#00E5FF]"
                >
                  <div className="flex justify-between items-center">
                ...
                <button 
                  ...
                  className="bg-[#00E5FF] text-black border-4 border-black p-4 text-xl font-bold uppercase tracking-wider transform -skew-x-5 hover:bg-[#FF4655] hover:text-white transition-colors shadow-[6px_6px_0px_#39FF14]"
                >
                  <div className="flex justify-between items-center">
                    <span>ADVANCED</span>
          ...
          /* CHAT INTERFACE */
          <>
            <div className="absolute top-4 right-4 bg-black border-2 border-[#00E5FF] px-3 py-1 font-bold text-xs uppercase text-[#00E5FF] tracking-wider z-10">
              Clearance: {experienceLevel}
            </div>
            
            <div className="flex-1 h-[500px] bg-[#0F1923] border-4 border-black mt-8 p-6 overflow-y-auto space-y-6 flex flex-col justify-start">
              {messages.length === 0 && (
